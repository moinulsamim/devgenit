...
import { clientLiveStatus, statusTones } from '../../../lib/service-status';
...
export default function ClientsPage() {
  ...
  const visible = clients.filter((client) => { const current = clientLiveStatus(client.services); const matchesFilter = filter === 'All' || labels[current] === filter; const term = query.toLowerCase(); return matchesFilter && (!term || `${client.name} ${client.company || ''} ${client.email}`.toLowerCase().includes(term)); });
  ...
  return <AdminShell breadcrumb="Clients"><main className="max-w-6xl mx-auto px-4 sm:px-8 py-6 sm:py-8">
    ...
    <section className="admin-card overflow-hidden">
      ...
      {loading ? <div className="animate-pulse h-56 bg-white" /> : visible.length === 0 ? <div className="p-12 text-center text-sm text-[var(--admin-muted)]"><Users className="mx-auto mb-2" size={20} />No matching clients.</div> : <div>{visible.map((client) => {
        const current = clientLiveStatus(client.services); const tones = statusTones[current]; const services = client.services || []; const amount = services.reduce((sum, service) => sum + Number(service.amount), 0); const pending = pendingClaimsCount(services);
        return <Link href={`/admin/clients/${client.id}`} key={client.id} className={`flex items-center gap-3 px-4 py-4 border-b last:border-0 border-[#edf2f0] ${tones.row}`}>
          <span className={`relative w-9 h-9 shrink-0 rounded-full text-[10px] font-semibold flex items-center justify-center ${tones.avatar}`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <p className={`text-xs font-semibold truncate ${tones.text}`}>{client.name}</p>
          ...
          <div className="w-24 hidden sm:block">
            <span className={`px-2 py-1 rounded-full text-[9px] font-semibold ${tones.badge}`}>{labels[current]}</span>
            <p className="text-[9px] text-[var(--admin-muted)] mt-1">{services.length} active services</p>
          </div>
          <div className="text-right w-20"><p className={`text-xs font-semibold ${tones.text}`}>${amount.toLocaleString()}</p><p className="text-[9px] text-[var(--admin-muted)]">monthly</p></div>
