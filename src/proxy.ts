...
export async function proxy(request) {
  ...
  if (pathname.startsWith('/api/client') && !pathname.startsWith('/api/client/login')) {
    ...
    if (!token || !(await verifyClientToken(token))) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }
  if (pathname.startsWith('/admin') && pathname !== '/admin/login') {
    const token = getCookie('admin_session');
    if (!token || !(await verifyAdminToken(token))) return NextResponse.redirect(new URL('/admin/login', request.url));
  }
  if (pathname.startsWith('/client') && pathname !== '/client/login') {
    const token = getCookie('client_session');
    if (!token || !(await verifyClientToken(token))) return NextResponse.redirect(new URL('/client/login', request.url));
  }
